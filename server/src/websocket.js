...
io.on("connection", (socket) => {
  ...
  socket.on("updatesocketid", async (data) => {
    await User.findByIdAndUpdate({ _id: data.userid }, { socketid: socket.id });
  ...
  socket.on("acceptRequest", async (data) => {
    ...
    await Request.deleteOne({ _id: data._id }, () =>
    ...
    io.of("/")
      ...
      .clients((error, socketIds) => {
        ...
        socketIds.forEach((socketId) =>
          io.sockets.sockets[socketId].leave(`room-${room}`)
        );
